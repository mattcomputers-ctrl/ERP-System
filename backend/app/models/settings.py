...
class SalesTaxOption(Base):
    """Configurable sales tax options that sync to QuickBooks."""
    __tablename__ = "sales_tax_options"
    id = Column(Integer, primary_key=True, index=True)
    name = Column(String(200), unique=True, nullable=False)  # e.g., "Taxable", "Non-Taxable", "Tax Exempt"
    description = Column(Text, nullable=True)
    qb_tax_code = Column(String(100), nullable=True)  # QuickBooks tax code reference
    is_active = Column(Boolean, default=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())
